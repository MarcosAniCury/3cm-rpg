...
class player(commands.Cog):
    ...
    async def criar_ficha(self, ctx):  # Criar player
        ...
        await asyncio.sleep(cooldown_comandos)
        await ctx.send("Habilidade gerada:__**" + habilidades['1']["habilidade"] + "**__",
                       delete_after=cooldown_comandos)
        await asyncio.sleep(cooldown_comandos)
        await ctx.send("Inicializando todos os atributos com valor **0**", delete_after=cooldown_comandos)
        await asyncio.sleep(cooldown_comandos)
        await ctx.send("**PLAYER CRIADO COM SUCESSO, SEU PROFILE IRÁ APARECER NA SUA FRENTE**",
                       delete_after=cooldown_comandos)
        await asyncio.sleep(cooldown_comandos)
    ...
    async def distribuir_pontos(self, ctx):
        ...
        while pontos_distribuir > 0:
            await ctx.send("Você possui **" + str(
                pontos_distribuir) + "** pontos para distribuir, digite qual o atributo e quantos pontos você deseja colocar (Digite **não** para cancelar):\nEX:**For 5**",
                           delete_after=60)
            ...
            if input_pontos.content.lower() == 'não' or input_pontos.content.lower() == 'n':
                ...
                player['atributos_variaveis']['pontos_atributos'] = str(
                    int(player['atributos_variaveis']['pontos_atributos']) - int(input[1]))
                pontos_distribuir = int(player['atributos_variaveis']['pontos_atributos'])
                await ctx.send(
                    "__Tem certeza que você vai adicionar **" + input[1] + "** pontos em **" + input[0] + "** (S/N):__",
                    delete_after=60)
                input_confirmacao = await self.client.wait_for('message', check=lambda msg: msg.author == ctx.author,
                                                               timeout=60)
                ...
                if input_confirmacao == 's' or input_confirmacao == 'sim':
                    ...
                    await ctx.send("distribuição de pontos bem sucedida, digite `3cm perfil` para verificar",
                                   delete_after=30)
                    await canal_log.send(
                        "O jogador " + ctx.author.mention + " adicionou **" + input[1] + "** pontos no atributo " +
                        input[0])
            else:
