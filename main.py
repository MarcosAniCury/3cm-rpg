...
async def load_all_cogs():
  for filename in os.listdir('Scripts/Cogs'):
    if filename.endswith('.py'):
      await client.load_extension(f'Scripts.Cogs.{filename[:-3]}'
                                  )  # Load cortando o .py do arquivo
...
)  # Quando digitar <prefix> help vai chamar a funcao
...
async def on_command_error(ctx, error):  # Tratamento de exceções
  if isinstance(error, commands.MissingRequiredArgument):
    ...
    await ctx.send(
      "Comando não encontrado, digite 3cm help para ver os comandos ativos",
      delete_after=20)
    ...
    await ctx.send("Erro encontrado, reporte a algum adm urgente:erro \"" +
                    error.args[0] + "\"",
                    delete_after=60)
    print(error)
    print("--------------------------------")
